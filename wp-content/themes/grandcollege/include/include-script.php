<?php
	...
	*/ 
	...
	function register_meta_script(){
		...
		}else if( $post_type == 'post' || $post_type == 'portfolio' || $post_type == 'gallery' || $post_type == 'course'){
	...
	*/ 
	...
	function register_non_admin_styles(){
		...
		// Page post_type
			...
			global $enable_footer_slideshow, $only_front_footer_slideshow;
			...
			$enable_footer_slideshow = get_option(THEME_SHORT_NAME.'_footer_enable_slideshow' ,'disable');
			$only_front_footer_slideshow = get_option(THEME_SHORT_NAME.'_footer_slideshow_frontpage' ,'enable');
			
			// If using slide show (elastislide)
			...
			if( strpos($gdl_page_xml,'<slider-type>Anything Slider</slider-type>') > -1  ||
				strpos($gdl_page_xml,'<Content-Slider>') > -1  ||
	...
	function register_non_admin_scripts(){
		...
		if (get_option(THEME_SHORT_NAME.'_enable_twitter_carousel','enable') == 'enable'){
			wp_deregister_script('cycle');
			wp_register_script('cycle', GOODLAYERS_PATH.'/javascript/jquery.cycle.js', false, '1.0', true);
