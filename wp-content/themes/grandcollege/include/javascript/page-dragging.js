...
jQuery(document).ready(function(){ 
	...
	var DIV_SIZE = [
		['1/4','element1-4',1/4,['Column','Personnal','Gallery','Content','Content-Slider','Blog','Page','Portfolio','Testimonial','Slider','Accordion','Tab','Divider','Message-Box','Toggle-Box','Price-Item']],
		['1/3','element1-3',1/3,['Column','Personnal','Gallery','Content','Content-Slider','Blog','Page','Portfolio','Testimonial','Slider','Accordion','Tab','Divider','Message-Box','Toggle-Box','Price-Item']],
		['1/2','element1-2',1/2,['Column','Personnal','Gallery','Content','Content-Slider','Blog','Page','Portfolio','Testimonial','Slider','Accordion','Tab','Divider','Message-Box','Toggle-Box','Price-Item']],
		['2/3','element2-3',2/3,['Column','Personnal','Gallery','Content','Content-Slider','Page','Portfolio','Testimonial','Slider','Accordion','Tab','Divider','Message-Box','Toggle-Box','Price-Item']],
		['3/4','element3-4',3/4,['Column','Personnal','Gallery','Content','Content-Slider','Page','Portfolio','Testimonial','Slider','Accordion','Tab','Divider','Message-Box','Toggle-Box','Price-Item']],
		['1/1','element1-1',1,['Column','Personnal','Gallery','Content','Content-Slider','Blog','Page','Portfolio','Testimonial','Slider','Accordion','Tab','Divider','Message-Box','Toggle-Box','Stunning-Text','Contact-Form','Price-Item']]
	];
	...
	// Content Slider
	jQuery('.page-item-content-slider').css('display','block');
	jQuery(".page-content-slider-add-more").click(function(){
		var added_tab = jQuery(this).siblings(".meta-input").children("#added-content-slider");
		var clone_tab = added_tab.find(".default").clone(true);
		clone_tab.attr('class','page-item-content-slider');
		clone_tab.find('textarea').attr('name', function(){
			return jQuery(this).attr('id') + '[]';
		});
		added_tab.siblings("#content-slider-num").val(function(){
			return parseInt(jQuery(this).val()) + 1;
		});
		added_tab.children("ul").append(clone_tab);
		added_tab.find('.page-item-content-slider').slideDown();
		
	});
	jQuery(".unpick-content-slider").click(function(){
		var deleted_tab = jQuery(this);
		...
		jQuery.confirm({
			...
			'buttons'	: {
				'Delete'	: {
					'class'	: 'confirm-yes',
					'action': function(){
						deleted_tab.parents('#page-item-content-slider').slideUp(function(){
							jQuery(this).remove();
						});
						deleted_tab.parents("#added-content-slider").siblings("#content-slider-num").val(function(){
							return parseInt(jQuery(this).val()) - 1;
						});
					}
				},
	...
	jQuery('select#page-option-top-slider-types').change(function(){
		if(jQuery(this).val()=='No Slider'){
			jQuery(this).parents('.meta-body').siblings('.open-top-slider-id').slideUp();
		}else{
			jQuery(this).parents('.meta-body').siblings('.open-top-slider-id').slideDown();
		}
	});
	jQuery('select#page-option-top-slider-types').each(function(){
		if(jQuery(this).val()=='No Slider'){
			jQuery(this).parents('.meta-body').siblings('.open-top-slider-id').css('display','none');
		}else{
			jQuery(this).parents('.meta-body').siblings('.open-top-slider-id').css('display','block');
		}
	});
	// Bind Under Top Slider
	jQuery('select#page-option-enable-under-slider').change(function(){
		if(jQuery(this).val()=='No'){
			jQuery(this).parents('.meta-body').siblings('.open-under-slider-id').slideUp();
		}else{
			jQuery(this).parents('.meta-body').siblings('.open-under-slider-id').slideDown();
		}
	});
	jQuery('select#page-option-enable-under-slider').each(function(){
		if(jQuery(this).val()=='No'){
			jQuery(this).parents('.meta-body').siblings('.open-under-slider-id').css('display','none');
		}else{
			jQuery(this).parents('.meta-body').siblings('.open-under-slider-id').css('display','block');
		}
	});
